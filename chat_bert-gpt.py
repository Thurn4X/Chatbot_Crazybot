...
# Il est bon de garder une normalisation minimale
...
def normalizeString(s):
    ...
    # Garde une normalisation simple
    s = re.sub(r"([.!?])", r" \1", s)
...
def chat():
    ...
    while True:
        try:
            ...
            # 2. Générer une réponse (avec des paramètres de sampling)
            outputs = model.generate(
                ...
                # --- NOUVEAUX PARAMÈTRES ---
                do_sample=True,       # Active l'échantillonnage (n'est plus "glouton")
                top_k=10,             # Ne considère que les 50 mots les plus probables
                top_p=0.95,           # Ne considère que les mots formant 95% de la probabilité
                temperature=0.8,      # Augmente la créativité (1.0 = normal, <1.0 = plus prudent, >1.0 = plus fou)
                no_repeat_ngram_size=2, # Empêche le bot de répéter des paires de mots
                # --------------------------

                num_return_sequences=1, # On ne veut qu'une réponse
